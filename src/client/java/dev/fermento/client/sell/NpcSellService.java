package dev.fermento.client.sell;
...
public final class NpcSellService {
	...
	public void start(String itemQuery) {
		if (itemQuery == null || itemQuery.isBlank()) {
			chat("Give an item name. Example: /fermento sell enchanted_wheat 5", ChatFormatting.RED);
			return;
		}
		if (running()) {
			chat("Sell already running. /fermento sell cancel to stop.", ChatFormatting.RED);
			return;
		}
		...
		FermentoMod.LOGGER.info("NPC sell start item={} repeats={}", targetId, repeatCount);
	}
		...
		switch (phase) {
			...
			case WAIT_MENU -> {
				if (isNpcMenuOpen(client)) {
					...
					phase = Phase.SELLING;
					ticksInPhase = 0;
					FermentoMod.LOGGER.debug("NPC sell round {}/{} slots={}", currentRound, repeatCount, slotsToClick.size());
	...
	private void beginRound(Minecraft client, LocalPlayer player, boolean first) {
		...
		FermentoMod.LOGGER.debug("NPC sell /gfs {} round {}/{} first={}", gfsName, currentRound, repeatCount, first);
	...
	private void recheckRemaining(Minecraft client, LocalPlayer player) {
		...
		if (recheckCount > MAX_RECHECKS) {
			FermentoMod.LOGGER.debug("NPC sell leftover slots={} after rechecks", slotsToClick.size());
			finishRound(client, soldThisRound);
			return;
		}
		nextSlotIndex = 0;
	...
	private static void chat(String message, ChatFormatting color) {
		...
		client.player.sendSystemMessage(
				Component.literal("[Fermento] " + message).withStyle(color)
		);
	}
