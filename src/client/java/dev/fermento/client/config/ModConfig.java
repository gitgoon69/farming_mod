package dev.fermento.client.config;
...
public final class ModConfig {
	...
	private static final Path PATH = FabricLoader.getInstance().getConfigDir().resolve("fermento.json");
	private static final Path LEGACY_PATH = FabricLoader.getInstance().getConfigDir().resolve("farmingprofit.json");
	...
	public static ModConfig load() {
		...
		try (Reader reader = Files.newBufferedReader(source)) {
			...
			FermentoMod.LOGGER.warn("Could not read {}", source.getFileName(), e);
	...
	public void save() {
		try {
			...
			FermentoMod.LOGGER.warn("Could not write fermento.json", e);
		}
	}
