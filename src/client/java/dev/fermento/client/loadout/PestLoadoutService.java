package dev.fermento.client.loadout;
...
import dev.fermento.client.compat.ClientMissTime;
...
public final class PestLoadoutService {
	...
	public void onChat(String raw) {
		...
		FermentoMod.LOGGER.info("Pest spawn detected, Farm loadout in {}ms", delayMs);
	}
	...
	public void tick(Minecraft client) {
		...
		switch (phase) {
			...
			case CLICK -> {
				...
				FermentoMod.LOGGER.info("Loadout {} left-click slot {}", pendingTarget, slotId);
				phase = Phase.CLOSING;
				ticksInPhase = 0;
			}
			case CLOSING -> {
	...
	private static void sendSetSpawn() {
		...
		FermentoMod.LOGGER.info("Pest spawn detected, /setspawn");
	}
