...
public final class UsagePingService {
	...
	private final ExecutorService executor = Executors.newSingleThreadExecutor(runnable -> {
		Thread thread = new Thread(runnable, "farmingprofit-usage");
	...
	private void ping(UUID uuid, String username, String version) {
		try {
			...
			if (status < 200 || status >= 300) {
				FarmingProfitMod.LOGGER.debug("Usage ping HTTP {}: {}", status, response.body());
			}
		} catch (InterruptedException e) {
			Thread.currentThread().interrupt();
		} catch (Exception e) {
			FarmingProfitMod.LOGGER.debug("Usage ping: {}", e.toString());
		}
	}
}
